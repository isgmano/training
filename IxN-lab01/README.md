...
2.  From **New Topology** add **EVPN VXLAN** on first port.

![Figure 1 Add EVPN](images/figure-01-add-evpn.png)

**Figure 1:** Add EVPN

3.  Rename the topology as **“Egress”** and device group as **“PE1-PE2”**.
4.  Also, set the device multiplier to **1**.

![Figure 2 Adjust Device Multiplier](images/figure-02-adjust-device-multiplier.png)

**Figure 2:** Adjust Device Multiplier

5.  Similarly, add EVPN VXLAN device group on other port.
6.  Rename the topology as **“Ingress”** and device group as **“PE3”**.
7.  Also, set the device multiplier to **1**.

![Figure 3 Base Topology](images/figure-03-base-topology.png)

**Figure 3:** Base Topology

> At this moment, if you are asking, where are two PEs in Egress, you are following **well**.

8.  Now add an IPv4 prefix to the EVPN VXLAN of Egress Topology.

![Figure 4 Choose Network Group](images/figure-04-choose-network-group.png)

**Figure 4:** Choose Network Group

![Figure 5 Select IPv4 Address Pools](images/figure-05-select-ipv4-address-pools.png)

**Figure 5:** Select IPv4 Address Pools

9.  Choose **IPv4 Address Pools** and click **Finish**.
10. Rename the network group as **“MH T5”** to indicate this network group will generate type 5 routes and this prefix is multi-homed (MH). Set IP address to **12.12.12.12/32**.
...
**Figure 6:** Type 5 attached to EVPN

11. Add an IPv4 Address pool to Ingress Topology.
...
**Figure 7:** Enhanced Topology with Traffic Endpoints

---

## Configure Connectivity
12. From the left pane click on **IPv4**. It will open the view to configure IPv4 addresses. Set:
...
**Figure 8:** IP Connectivity

13. Select **BGP Peer** from left pane and set:
...
**Figure 9:** Configure BGP Peership

14. Also, from same **BGP Peer** view, choose **Learn Route Filters** tab and enable **EVPN Filters**.

![Figure 10 Set Learned Info Filter](images/figure-10-set-learned-info-filter.png)

**Figure 10:** Set Learned Info Filter
...
**Figure 11:** Interfaceless IP-VRF-To-IP-VRF Model
...
15. Return to **BGP Peer** in left pane. Now choose **EVPN** tab. Set:
...
**Figure 12:** Creation of Ethernet Segments

16. Now, click on **BGP Ethernet Segment** tab. Set ESI value for Egress topology to:
  - `0x00 00 00 00 00 00 00 0A BC`
  - Leave the values for second port.

![Figure 13 Set ESI Value](images/figure-13-set-esi-value.png)

**Figure 13:** Set ESI Value

17. Stay in the same tab and scroll towards right. Look for **Support L3 Fast Convergence** field. Enable for all ports.

![Figure 14 Enable L3 Fast Convergence](images/figure-14-enable-l3-fast-convergence.png)

**Figure 14:** Enable L3 Fast Convergence

18. Enable **Advertise IP Aliasing automatically** only for **PE2**.
...
**Figure 15:** Advertise IP Aliasing Automatically

### Next-hop tweaks (essential trick)
19. Come to **Advanced** tab. Now set next hop for **PE2 (1.2)** manually to **30.30.30.30**.
...
**Figure 16:** Nexthop in Ethernet Segment

20. From left pane select **EVI VXLAN**, reach to **Advance** tab and modify the nexthop in similar fashion.

![Figure 17 Nexthop in EVI](images/figure-17-nexthop-in-evi.png)

**Figure 17:** Nexthop in EVI

### Route targets and labels
21. From left pane select **EVI VXLAN** and stay in **Basic** sub tab of **EVI** tab.
- Enable **Enable L3 Target List** for both sides.
22. Scroll towards right. Set **IP AD Route Label** to:
  - **6534** (PE1)
  - **7534** (PE2)
  - **8534** (PE3)
...
**Figure 18:** L3 RT and IP AD Label

23. Stay in the same tab. Scroll to find the following fields:
...
**Figure 19:** Enable L2 Ext Comm, Primary PE, and Backup
...
24. Set **L3 Export Route Target List**. Set the assigned number to **1** for **PE2**.

![Figure 20 Set L3 RT](images/figure-20-set-l3-rt.png)

**Figure 20:** Set L3 RT

25. Set route label value for **Egress** and **Ingress** as **2001**.
  - Navigate: **EVI VXLAN → IPv4 Routes → Label Space / SRv6 SID** tab.

![Figure 21 Set Prefix Label](images/figure-21-set-prefix-label.png)

**Figure 21:** Set Prefix Label

26. The prefix from Egress side should be advertised only by **PE1** (primary PE).
...
**Figure 22:** Advertise Prefix using PE1 only

27. At point 19, we added an arbitrary next hop address **30.30.30.30** for PE2.
...
**Figure 23:** Advertise Nexthop PE2
...
**Figure 24:** All are Green!

28. Select **BGP Peer** of **PE3**.
  - Go to **BGP Peer Grid** at **Basic** subtab.
  - Right click: **Get VPN Learned Info → Get EVPN Learned Info**.


![Figure 25 Fetch EVPN Learned Info](images/figure-25-evpn-fetch-learned-info.png)

**Figure 25:** Fetch EVPN Learned Info

29. From the learned information we focus on:
  - **EVPN Ethernet AD**
  - **EVPN IP Prefix**

![Figure 26 EVPN Ethernet AD](images/figure-26-evpn-ethernet-ad.png)

**Figure 26:** EVPN Ethernet AD

![Figure 27 EVPN IP Prefix](images/figure-27-evpn-ip-prefix.png)

**Figure 27:** EVPN IP Prefix
...
30. Add **L2-3 Traffic Items**. This opens endpoint selection.
...
**Figure 28:** Endpoint Selection

31. MH T5 is multihomed to two PEs, so there should be a single endpoint.
...
**Figure 29:** Traffic Rectangle

32. Click on the arrow to update the selected endpoint set.
  - Check the encapsulations and click **Next**.

![Figure 30 Final Endpoint set](images/figure-30-final-endpoint-set.png)

**Figure 30:** Final Endpoint set

33. Proceed through Encapsulation → Flow Group → Frame Setup → Rate Setup → Flow Tracking.
34.	From flow group setup, upon clicking next we’ll come to frame setup. Choose from various options to define the frame. And click Next.
35. 35.	Now, we are at Rate setup. We can choose various options to define the rate and behavior of the traffic.
36. In **Flow tracking**, enable:
  - **Source/Dest Value Pair**
  - **VXLAN: VNI**

37. Move to **Protocol Behaviors**.

![Figure 31 Protocol Behaviors](images/figure-31-protocol-behaviors.png)

**Figure 31:** Protocol Behaviors

38. Ordinal value for next hop selection is important:
  - **Ordinal = 0** → VNI **2001**
  - **Ordinal = 1** → VNI **7534**

39. In **Preview**, click **View Flow Groups/Packet**, select the flow group, and verify VNI.

**Ordinal Value = 0**

![Figure 32 Traffic Preview for Ordinal = 0](images/figure-32-traffic-preview-ordinal-0.png)

**Figure 32:** Traffic Preview for Ordinal = 0

**Ordinal Value = 1**

![Figure 33 Traffic Preview for Ordinal = 1](images/figure-33-traffic-preview-ordinal-1.png)

**Figure 33:** Traffic Preview for Ordinal = 1

40. Finish to create the traffic item.
41. Traffic item will look like follows.

![Figure 34 Traffic Item](images/figure-34-traffic-item.png)

**Figure 34:** Traffic Item

42. Apply traffic.

![Figure 35 Apply Traffic](images/figure-35-apply-traffic.png)

**Figure 35:** Apply Traffic

43. Start traffic.
44. In the statistics, you should find the trackers in yellow (as defined earlie in point 36).

![Figure 36 Traffic Stats](images/figure-36-traffic-stats.png)

**Figure 36:** Traffic Stats
